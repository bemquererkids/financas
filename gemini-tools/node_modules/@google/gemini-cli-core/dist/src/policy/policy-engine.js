    ...
    async check(toolCall, serverName) {
        ...
        for (const rule of this.rules) {
            ...
            if (match) {
                ...
                if (isShellCommand) {
                    const shellResult = await this.checkShellCommand(toolCall.name, command, rule.decision, serverName, shellDirPath, rule.allowRedirection, rule);
        ...
        if (!decision) {
            ...
            if (isShellCommand && decision !== PolicyDecision.DENY) {
                const shellResult = await this.checkShellCommand(toolCall.name, command, decision, // default decision
                serverName, shellDirPath, false, // no rule, so no allowRedirection
                undefined);
                decision = shellResult.decision;
                matchedRule = shellResult.rule;
            }
