...
import { A2AClientManager, createAdapterFetch, } from './a2a-client-manager.js';
...
describe('A2AClientManager', () => {
    ...
    describe('createAdapterFetch', () => {
        it('normalizes TASK_STATE_ enums to lower-case', async () => {
            const baseFetch = vi
                .fn()
                .mockResolvedValue(new Response(JSON.stringify({ status: { state: 'TASK_STATE_WORKING' } })));
            const adapter = createAdapterFetch(baseFetch);
        ...
        it('lowercases non-prefixed task states', async () => {
            const baseFetch = vi
                .fn()
                .mockResolvedValue(new Response(JSON.stringify({ status: { state: 'WORKING' } })));
            const adapter = createAdapterFetch(baseFetch);
            const response = await adapter('http://example.com', {
                method: 'POST',
                body: '{}',
        ...
        it('bypasses adapter for JSON-RPC requests', async () => {
            ...
            expect(baseFetch).toHaveBeenCalledWith('http://example.com', expect.objectContaining({ body: rpcBody }));
        });
        it('applies dialect translation for remote REST requests', async () => {
            ...
            expect(baseFetch).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ body: expectedBody }));
        });
    });
});
